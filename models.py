...
class NearEarthObject:
    ...
    def __init__(self, designation='', name='', diameter=float('nan'), hazardous=''):
        ...
        """
        try:
            if not type(designation) is str:
                raise TypeError("designation --> string.")

            if not type(name) is str:
                raise TypeError("name --> string.")

            if not (type(diameter) is float or type(diameter) is int):
                raise TypeError("diameter --> number.")

            if not (type(hazardous) is bool or type(hazardous) is str):
                raise TypeError("hazardous --> true/false or Y/N.")
    ...
    def __str__(self):
        """Return `str(self)`."""
        hazardous_text = "is" if self.hazardous else "is not"
        return f"NEO {self.fullname} has a diameter of {self.diameter:.3f} km and {hazardous_text} potentially hazardous."
    ...
    def serialize(self):
        """Json serialization of NearEarthObject"""
        return {'designation': self.designation, 'name': self.name or '',
                'diameter_km': self.diameter or 'nan',
...
class CloseApproach:
    ...
    def __init__(self,  designation='', time='', distance=float('nan'), velocity=float('nan'), neo=None):
        ...
        """
        try:
            self._designation = str(designation) if designation else None
            ...
            print(f"Error occurred: {e}. Please insert the right type for each attribute.")
    ...
    def json_serialize(self):
        """Json serialization of CloseApproach"""
    ...
    def csv_serialize(self):
        """CSV serialization of CloseApproach """
        return {'datetime_utc': datetime_to_str(self.time), 'distance_au': self.distance,
                'velocity_km_s': self.velocity, **self.neo.serialize()}